...
int __io_getchar(void) {
	...
	USART_GetFlagStatus(USART2, USART_FLAG_ORE) == SET) {
		if (USART_GetFlagStatus(USART2, USART_FLAG_PE) == SET) print_str("[ERROR: Parity]\r\n");
		if (USART_GetFlagStatus(USART2, USART_FLAG_FE) == SET) print_str("[ERROR: Framing]\r\n");
		if (USART_GetFlagStatus(USART2, USART_FLAG_NE) == SET) print_str("[ERROR: Noise]\r\n");
		if (USART_GetFlagStatus(USART2, USART_FLAG_ORE) == SET) print_str("\n[ERROR: Overrun]\r\n");
		
		GPIO_SetBits(GPIOC, GPIO_Pin_7);
		delay(2000000);
		GPIO_ResetBits(GPIOC, GPIO_Pin_7);
		
		volatile uint32_t dummy_sr = USART2->SR;
		volatile uint32_t dummy_dr = USART2->DR;
